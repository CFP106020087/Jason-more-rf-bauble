...
public class IntelDataHelper {
     ...
     * 增加玩家对特定生物的情报等级
     */
    public static void incrementIntelLevel(@Nullable IHumanityData data, ResourceLocation entityId) {
        if (data == null || entityId == null) return;
        data.setIntelLevel(entityId, data.getIntelLevel(entityId) + 1);
    }
    ...
    public static void setIntelLevel(@Nullable IHumanityData data, ResourceLocation entityId, int level) {
        if (data == null || entityId == null) return;
        data.setIntelLevel(entityId, Math.max(0, level));
    }

    /**
     * 计算对特定生物的伤害加成倍率
     * @return 1.0 = 无加成, 1.1 = +10%, 1.5 = +50%, 等
     */
    public static float calculateDamageMultiplier(@Nullable IHumanityData data, ResourceLocation entityId) {
        ...
        // 每级 +10% 伤害
        return 1.0f + (level * ItemIntelBook.DAMAGE_BONUS_PER_BOOK);
    }
