...
/**
 * 时序织印 - First Aid 模组兼容层
 * Temporal Fabric - First Aid Mod Compatibility Layer
 *
 * First Aid 模组有独立的死亡逻辑：当 HEAD 或 BODY 血量到 0 时触发死亡
 * 通过订阅 FirstAidLivingDamageEvent 并直接修复部位血量来防止死亡
 */
@Mod.EventBusSubscriber(modid = moremod.MODID)
public class TemporalFirstAidCompat {
    ...
    public static void onFirstAidDamage(FirstAidLivingDamageEvent event) {
        try {
            ...
            if (!FabricEventHandler.hasTemporalFabric(player)) return;

            // 回溯保护期间完全免疫 First Aid 伤害
