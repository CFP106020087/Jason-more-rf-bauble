...
### ✅ EnergyPunishment（能量惩罚系统）
- **整合位置**: `ModuleTickHandler.handleEnergyPunishment()`
- **实现方式**: 基于 IMechCoreData 能量状态调用惩罚系统
- **触发条件**:
  * EMERGENCY (能量 < 5%): 模块降级 + 装备损坏
  * CRITICAL (能量 < 0%): DOT 伤害 + 自毁倒计时
- **能量阈值**:
  * NORMAL: ≥30%
  * POWER_SAVING: ≥15% (高耗能功能降低)
  * EMERGENCY: ≥5% (非必要系统关闭)
  * CRITICAL: <5% (仅保留生存系统)
- **惩罚效果**:
  * DOT 伤害: 1 HP/tick
  * 模块降级: 2 个随机模块/10s
  * 装备损坏: 护甲和武器耐久 -10/10s
  * 自毁倒计时: 20 秒（10s/5s 警告）
- **兼容性**: 桥接旧 ItemStack 系统（`EnergyPunishmentSystem.tick()`）
