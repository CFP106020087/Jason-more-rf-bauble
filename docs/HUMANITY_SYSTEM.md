...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `dissolutionDuration` | 60 秒 | 崩解持续时间 |
| `dissolutionDamageInterval` | 5 秒 | 伤害间隔 |
| `dissolutionDamagePercent` | 0.1 (10%) | 每次伤害占最大生命值的百分比 |
...
| 行为 | 消耗量 | 说明 |
|------|--------|------|
| 睡眠剥夺 | -0.004/秒 | 72分钟不睡觉后触发 |
| 击杀被动生物 | -0.1 | 单次 |
| 击杀村民 | -0.5 | 单次 |
| 战斗状态 | -0.02/秒 | <50%人性时持续消耗 |

#### ⚠️ 仅配置 (未实现)

| 行为 | 配置值 | 说明 |
|------|--------|------|
| ~~虚空发生器~~ | 0.5 | 配置存在但无调用代码 |
| ~~裂隙脉冲~~ | 1.0 | 配置存在但无调用代码 |
| ~~相位转移~~ | 0.4 | 配置存在但无调用代码 |
| ~~维度跳跃~~ | 1.5 | 配置存在但无调用代码 |
| ~~超频运行~~ | 0.01/秒 | 配置存在但无调用代码 |
| ~~异常维度~~ | 0.006/秒 | 配置存在但无调用代码 |
...
| 人性值 | 交互等级 | 效果 |
|--------|---------|------|
| ≥ 80% | TRUSTED | 可交互，价格 ×0.85 (-15% 折扣) |
| 60-80% | NORMAL | 可交互，正常价格 |
| 40-60% | SUSPICIOUS | 可交互，价格 ×1.5 (+50% 加价) |
| 25-40% | HOSTILE | 村民恐惧拒绝，无法交互 |
| < 25% | INVISIBLE | 村民完全无视，无法交互 |
...
| 人性值 | 状态 | HP | NPC价格 | 链结站 | 档案槽 |
|--------|------|-----|---------|--------|--------|
| 80%+ | 猎人协议 | 100% | 0.85x折扣 | ✓ | 8-10 |
| 60-80% | 高人性 | 100% | 1.0x正常 | ✓ | 6-7 |
| 40-60% | 灰域 | 100% | 1.5x加价 | ✗ | 4-5 |
