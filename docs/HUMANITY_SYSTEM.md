...
| 人性值 | 交互等级 | 实际效果 |
|--------|---------|---------|
| ≥ 80% | 信任 (TRUSTED) | 可交互 ~~(折扣未实现)~~ |
| 60% - 80% | 正常 (NORMAL) | 可交互 |
| 40% - 60% | 怀疑 (SUSPICIOUS) | 可交互 ~~(加价未实现)~~ |
| 25% - 40% | 敌对 (HOSTILE) | ✅ 拒绝交易 + 村民逃跑 |
| < 25% | 无视 (INVISIBLE) | ✅ 完全无法交互 |

> ⚠️ 注：价格倍率功能 (`getTradePriceMultiplier`) 代码存在但未接入交易系统
...
|------|--------|------|
| ~~熟食~~ | 0.05 | 方法存在但无事件调用 |
| ~~精致食物~~ | 0.15 | 方法存在但无事件调用 |
| ~~金质食物~~ | 0.3 | 方法存在但无事件调用 |
| ~~村民交易~~ | 0.3 | 方法存在但无事件调用 |
| ~~收获作物~~ | 0.05 | 方法存在但无事件调用 |
...
| 等级 | 配置值 | 说明 |
|------|--------|------|
| ~~TRUSTED~~ | 0.85x | `getTradePriceMultiplier()`存在但未被调用 |
| ~~SUSPICIOUS~~ | 1.5x | 价格修改功能未接入交易系统 |
...
|--------|------|-----|-----|--------|--------|
| 80%+ | 猎人协议 | 100% | 可交互 | ✓ | 8-10 |
| 60-80% | 高人性 | 100% | 可交互 | ✓ | 6-7 |
| 40-60% | 灰域 | 100% | 可交互 | ✗ | 4-5 |
